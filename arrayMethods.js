...
// CODE HERE
const postTaxPrices = prices.map(taxes => taxes + (taxes * 0.07));
...
const totalPrices = orders.map(function(totalPrice){
  return ((totalPrice.price * totalPrice.tax) + totalPrice.price)
})
...
const bobsPrices = purchases.filter(function(elem){
